...
- **Durum:** 🟢 Çalışıyor
- **Notlar:** Test edildi. İnteraktif menü, çoklu seçim mekanizmaları ve `--auto` (otomatik mod) akışı kararlı ve sorunsuz çalışıyor.
...
### `scripts/dotfiles.sh`
- **Durum:** 🟢 Çalışıyor
- **Notlar:** Test edildi. `check`, `diff`, `deploy` ve `pull` mekanizmaları son derece kararlı çalışıyor. Atomik yedekleme sistemi ve `.keep` koruması hatasız işliyor.
- **Yapılacaklar:** Şimdilik bir şey yok.
...
### `scripts/shell.sh`
- **Durum:** 🟢 Çalışıyor
- **Notlar:** Zsh, Oh My Zsh ve eklentilerin kurulumunu gerçekleştiriyor. Katı hata kontrolleri ve güvenli kurulum akışı sorunsuz tamamlanıyor.
- **Yapılacaklar:** Şimdilik bir şey yok.
...
- **Durum:** 🟢 Çalışıyor
- **Notlar:** Ekran dondurma (`wayfreeze`), alan seçme (`slurp`), düzenleme (`satty`), pencere ve tam ekran özellikleri başarıyla test edildi ve kusursuz çalışıyor.
- **Yapılacaklar:** Şimdilik bir şey yok.

### `config/hypr/scripts/theme-switch.sh`
- **Durum:** 🟢 Çalışıyor
- **Notlar:** Farklı masaüstü temaları ve paletler (Catppuccin, Nord, vb.) arasında dinamik ve kararlı geçiş sağlıyor.
- **Yapılacaklar:** Şimdilik bir şey yok.
